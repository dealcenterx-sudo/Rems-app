...
- [x] **TEST-03**: CI runs rules + API suites alongside existing lint → test → build, without disturbing CRA's Jest setup
...
- [x] **OBS-02**: Serverless function errors are captured via a `withSentry` wrapper that flushes before responding
