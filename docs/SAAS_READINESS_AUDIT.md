...
| Medium | Handled serverless 500s/502s are not reported to Sentry — `withSentry` only captures errors a handler THROWS, but wrapped handlers catch their own errors internally and return a response without re-throwing. | `api/_lib/withSentry.js` (the catch block only runs on a thrown error); `api/accept-invite.js`, `api/lead-intake.js`, `api/send-email.js` (each catches internally and returns 500/502 without re-throwing). | Gracefully-handled serverless failures never reach Sentry; OBS-02 is satisfied only for truly UNCAUGHT throws. Normal error paths (bad payloads, provider failures, 502s) stay invisible in production monitoring. | Instrument each handler's catch block to call a shared `captureException` before returning 500/502 (the "loud, not silent" pattern), keeping 400/401/405 client-error paths un-reported. | Open — surfaced by Phase 3 verification; Phase 3 intentionally accepts current wrapper semantics (OBS-02 met via uncaught-throw coverage in `tests/api/withSentry.test.mjs`). Routed to Phase 5 / DATA-02 follow-up (loud-not-silent theme). |
