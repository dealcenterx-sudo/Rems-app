        ...
        {data.map((item, idx) => (
          <div key={idx} style={{ 
            ...
            <div style={{
              ...
              background: `linear-gradient(180deg, ${item.color || '#00ff88'}, ${item.color || '#00ff88'}88)`,
      ...
      <div style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
        <div style={{ 
          ...
          background: `conic-gradient(
            ${data.map((item, idx) => {
              const prevTotal = data.slice(0, idx).reduce((sum, d) => sum + d.value, 0);
              const percentage = (item.value / total) * 100;
              const prevPercentage = (prevTotal / total) * 100;
...
const MetricCard = ({ label, value, change, icon, color }) => (
  <div style={{ 
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <div style={{ fontSize: '12px', color: '#888888', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
        {label}
      </div>
      <div style={{ 
        width: '36px', 
...
const CRMDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Conversion rate (deals / sellers)
      const conversionRate = sellers.length > 0 
        ? ((deals.length / sellers.length) * 100).toFixed(1)
        : 0;
      ...
      setStats({
        ...
        avgDealValue: 0 // Can add later if you store deal values
      });
  ...
  return (
    <div className="page-content">
      ...
      <div style={{ 
        ...
        <h3 style={{ fontSize: '16px', color: '#ffffff', marginBottom: '20px', fontWeight: '600' }}>
          Quick Summary
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          <div>
            <div style={{ fontSize: '12px', color: '#888888', marginBottom: '8px' }}>Sellers</div>
            <div style={{ fontSize: '24px', color: '#00ff88', fontWeight: '700' }}>{stats.totalSellers}</div>
          </div>
          <div>
            <div style={{ fontSize: '12px', color: '#888888', marginBottom: '8px' }}>Buyers</div>
            <div style={{ fontSize: '24px', color: '#0088ff', fontWeight: '700' }}>{stats.totalBuyers}</div>
          </div>
          <div>
            <div style={{ fontSize: '12px', color: '#888888', marginBottom: '8px' }}>Closed Deals</div>
            <div style={{ fontSize: '24px', color: '#aa00ff', fontWeight: '700' }}>
              {stats.dealsByStatus.find(d => d.label === 'Closed')?.value || 0}
            </div>
          </div>
          <div>
            <div style={{ fontSize: '12px', color: '#888888', marginBottom: '8px' }}>Active Deals</div>
            <div style={{ fontSize: '24px', color: '#00ff88', fontWeight: '700' }}>
              {stats.dealsByStatus.find(d => d.label === 'Active')?.value || 0}
            </div>
          </div>
        </div>
      </div>
